...
- [~] (T3·C) **1D-2.** PARTIAL 2026-04-29 → 2026-05-04.

  **Iter 3 — abandoned 2026-05-04.** Restoring `_voicePlayer.stop()` before play and adding `PlayerState.completed` listener (intended to fix iter 2's silent world/arc voices) regressed the countdown — countdown voices use `interrupt: true` which already calls stop, and the additional pump-iteration stop confused iOS audioplayers state. Jim flagged it as the same pattern that bit Android development. Reverted before commit; iter 2 (commit `d8e4dcc`) remains the deployed state. See memory `feedback_audio_iterate_carefully.md` for the lesson. **Voice playback on iOS Simulator is known-flaky for rapid restart cycles; defer further investigation to real-device test in 2D-3 / 2E-1 (TestFlight).**

  - **Step 1 — `audioplayers` revert: APPROVED + IMPLEMENTED + VERIFIED.** `audioplayers ^6.6.0` → `^6.3.0` plus `dependency_overrides: audioplayers_darwin: 6.3.0` to force the transitive (caret resolution still permitted 6.4.0). Re-running 1D-1 confirmed: ZERO Swift continuation leaks (was 1×), ZERO `preload_partial` events (was 1×). Upstream issue filed at https://github.com/bluefireteam/audioplayers/issues/1982 documenting the leak so the broader community benefits.
  - **Step 2 — pre-existing iOS audio bugs in `lib/services/audio_service.dart`** (revealed by the same Simulator session, audible to Jim — were masked while the SDK-level Swift leak crashed playback entirely): three symptoms map to one root mechanism, all **Tier 3 — awaiting Jim's audio sign-off before implementing**.
    - **Symptom A (voice cut mid-sentence)** — when tapping BLAZE: hear "Let'..." then silence. Mechanism: `audio_service.dart:597` `await _voicePlayer.stop()` runs before each new voice play; combined with `audio_service.dart:604-607` `onPlayerStateChanged.where((s) => s == PlayerState.stopped)` listener returning `completed = false` immediately when iOS fires an early `stopped` event during the play lifecycle, the queue cycles to the next voice which calls `stop()` and kills the current one.
    - **Symptom B (victory voice cut, then total silence)** — first celebration phrase cuts mid-word, then no music, no SFX. Mechanism: same as A for the voice cut. Music silence is separate — something in the brushing→victory transition stops `_musicPlayer` and it's never restarted on the victory screen.
    - **Symptom C (no music on home after "Done")** — `home_screen.dart` doesn't re-trigger `AudioService().playMusic(...)` on return from victory; with `_musicPlayer` left in a stopped state from B, the entire app stays silent until the user taps BRUSH again.
  - **Proposed Tier 3 fixes (do NOT implement without Jim's nod):**
    1. **Drop the `PlayerState.stopped` race in `audio_service.dart:604-607`** — leave only `onPlayerComplete` + the 15s timeout in `Future.any`. The `stopped` event was a fallback that hurts more than it helps on iOS. On Android v20 production audio works regardless of this listener.
    2. **Stop calling `_voicePlayer.stop()` before each new voice play (line 597)** — `audioplayers`' `play()` handles source replacement. The `stop()` is what cuts the current voice when the queue advances. (Combined with fix #1, the queue won't advance until the current voice actually completes, so `stop()` becomes redundant.)
    3. **Restart music on home-screen resume + on victory entry** — `home_screen.dart` `initState` and victory screen `initState` should call `AudioService().playMusic('battle_music_loop.mp3')` (or equivalent gating: only if currently stopped). Today the music start is wired only into `brushing_screen.dart` start.
    4. Re-run 1D-1 after each fix to confirm zero audible cuts; commit each fix as its own atomic Tier 3 change so a regression on Android v21 can be bisected.
  - **Cycle 17 placement:** these are the highest-value Tier 3 audio items. Test against real iPhone hardware in 2D-3 / 2E-1 (TestFlight) before launch — Simulator audio is sometimes more glitchy than device.
