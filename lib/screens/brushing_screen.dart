...
class _BrushingScreenState extends State<BrushingScreen>
  ...
  WeekendEvent _weekendEvent = const WeekendEvent(
    isActive: false,
  ...
  Future<void> _loadHeroAndWorld() async {
    ...
    final dailyModifier = _worldService.getDailyModifier();
    final weekendEvent = _worldService.getWeekendEvent();
    final baseBossRule = totalBrushes > 0 && (totalBrushes + 1) % 5 == 0;
    final baseChance = 0.2;
    final chance =
        (baseChance *
                dailyModifier.bossChanceMultiplier *
                weekendEvent.bossChanceMultiplier)
            .clamp(0.0, 0.9);
    final roll = _random.nextDouble();
    final rollBoss = roll < chance;
    final isBossSession = baseBossRule || rollBoss;
    _telemetry.logEvent(
      'boss_session_roll',
      params: {
        'event_active': weekendEvent.isActive,
        'base_rule_hit': baseBossRule,
        'chance_percent': (chance * 100).round(),
        'roll_percent': (roll * 100).round(),
        'boss_session': isBossSession,
      },
    );
    if (mounted) {
      setState(() {
        ...
        _baseBossRuleHit = baseBossRule;
        _isBossSession = isBossSession;
  ...
  void _startBrushing({bool resumeFromCheckpoint = false}) {
    _telemetry.logEvent(
      ...
      params: {
        ...
        'weekend_event_active': _weekendEvent.isActive,
        'boss_session': _isBossSession,
        'base_boss_rule_hit': _baseBossRuleHit,
      },
    );
    ...
    Navigator.of(context).pushReplacement(
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) => VictoryScreen(
          ...
          weekendEventActive: _weekendEvent.isActive,
          weekendChestBonus: _weekendEvent.chestBonusStars,
        ),
