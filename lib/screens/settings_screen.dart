...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _loadSettings() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _streakShieldEnabled = prefs.getBool('streak_shield_enabled') ?? false;
  ...
  Future<void> _toggleStreakShield(bool value) async {
    final allowed = await _ensureParentAccess();
    if (!allowed) return;
    await _streakService.setStreakShieldEnabled(value);
  ...
  Future<void> _shareWeeklyProgressCard() async {
    final allowed = await _ensureParentAccess();
    if (!allowed) return;

    final totalWeekBrushes = _weeklyBrushCounts.fold<int>(0, (a, b) => a + b);
    final missions = await _missionService.getWeeklyMissions();
    final completedMissions = missions.where((m) => m.completed).length;
    final claimedMissions = missions.where((m) => m.claimed).length;

    final message =
        '''
Brush Quest - Weekly Progress Card

Total brushes (7 days): $totalWeekBrushes
AM brushes: $_weeklyMorningBrushes
PM brushes: $_weeklyEveningBrushes
Best streak: $_bestStreak
Lifetime brushes: $_totalBrushes
Weekly missions: $completedMissions/${missions.length} completed ($claimedMissions claimed)

Space Ranger progress is on track. Keep brushing twice a day!
''';

    await SharePlus.instance.share(
      ShareParams(text: message, subject: 'Brush Quest Weekly Progress'),
    );
    _telemetry.logEvent(
      'weekly_progress_shared',
      params: {
        'week_brushes': totalWeekBrushes,
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _SettingCard(
                      ...
                      title: 'Streak Shield (1 miss/week)',
                      child: Switch(
                        value: _streakShieldEnabled,
                        onChanged: _toggleStreakShield,
                    ...
                    _SettingCard(
                      ...
                      title: 'Share weekly progress card',
                      child: IconButton(
                        icon: const Icon(
                          Icons.send_rounded,
                        ...
                        onPressed: _shareWeeklyProgressCard,
                      ),
                    ),
